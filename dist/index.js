...
function askCreateGame() {
    ...
    const genreInput = prompt(`Genero (${GENRES.join("/")}): `).trim();
...
function askUpdateGame() {
    ...
    const genreInput = prompt(`Nuevo genero (${GENRES.join("/")}, vacio=mantener): `).trim();
    const launch = prompt("Nueva fecha (YYYY-MM-DD, vacio=mantener): ").trim();
    const multiRaw = prompt("Multijugador? (si/no, vacio=mantener): ").trim();
    const formatInput = prompt(`Nuevo formato (${FORMATS.join("/")}, vacio=mantener): `).trim();
    try {
